...
const FloatingChatbot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {/* Floating bubble */}
      <motion.button
        className="relative w-16 h-16 rounded-full bg-gradient-to-br from-cyan-400 to-pink-400 shadow-2xl flex items-center justify-center border-4 border-cyan-400 hover:scale-110 transition-all duration-300"
        style={{ boxShadow: open ? '0 0 40px #00FFE7' : '0 0 20px #00FFE7' }}
        onClick={() => setOpen(v => !v)}
        initial={{ scale: 1 }}
        animate={{ scale: open ? 1.08 : 1 }}
        whileTap={{ scale: 0.95 }}
      >
        <img src={avatarUrl} alt="Vaanee" className="w-10 h-10 rounded-full object-cover border-2 border-white shadow-md" />
        {/* {isTyping && <Lottie animationData={waveformAnim} loop className="absolute bottom-1 left-1 w-12 h-4" />} */}
        <span className="absolute bottom-2 right-2 w-3 h-3 bg-green-400 rounded-full border-2 border-white animate-pulse" />
      </motion.button>
    </div>
  );
};
